...
import org.jabref.gui.edit.TranslateAbstractAction;
...
public class EditorMenus {
    ...
    /**
     * The default context menu with a specific menu for translating abstracts into German
     *
     * @param textArea text-area that this menu will be connected to
     * @param dialogService provides user dialoge service
     * @return menu containing items of the default menu and an item for translating abstracts into German
     */
    public static Supplier<List<MenuItem>> getAbstractMenu(TextArea textArea, DialogService dialogService) {
        return () -> {
            CustomMenuItem abstractMenuItem = new CustomMenuItem(new Label(Localization.lang("Translate"), new FontIcon(MaterialDesignT.TRANSLATE)));
            TranslateAbstractAction action = new TranslateAbstractAction(textArea, dialogService);
            List<MenuItem> menuItems = new ArrayList<>(6);
            if(textArea.getLength() > 0) {
                menuItems.add(abstractMenuItem);
                Tooltip.install(abstractMenuItem.getContent(), new Tooltip(Localization.lang("Translate this text into German.")));
                abstractMenuItem.setOnAction(event -> action.execute());
            }
